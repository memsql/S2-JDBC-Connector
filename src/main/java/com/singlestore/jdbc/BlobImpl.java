...
public class BlobImpl implements Serializable {
   ...
   * @param length length
   */
  public BlobImpl(byte[] bytes, int offset, int length) {
    if (bytes == null) {
      throw new IllegalArgumentException("byte array is null");
    }
    data = bytes;
  ...
  private BlobImpl(int offset, int length, byte[] bytes) {
    this.data = bytes;
    this.offset = offset;
    this.length = length;
  }

  public static BlobImpl safeSingleStoreBlob(byte[] bytes, int offset, int length) {
    return new BlobImpl(offset, length, bytes);
  }
  ...
  static class BlobOutputStream extends OutputStream {
    ...
    private final BlobImpl blob;
    private int pos;

    public BlobOutputStream(BlobImpl blob, int pos) {
      this.blob = blob;
      this.pos = pos;
    }
