...
public class Statement implements java.sql.Statement {
   ...
   * silently dropped on JDBC side. To apply the limit on the database side, update sql select limit
   * session engine variable {@link Connection#setSqlSelectLimit}.
   *
   * @param max the new max rows limit; zero means there is no limit
  ...
  public void setLargeMaxRows(long max) throws SQLException {
    ...
    if (con.getSqlSelectLimit() != 0 && max > con.getSqlSelectLimit()) {
      logger.warn(
          "MaxRows value of {} is ignored because the current connection has a lower sql_select_limit property set to {}",
          max,
          con.getSqlSelectLimit());
    }
    this.maxRows = max;
  }
