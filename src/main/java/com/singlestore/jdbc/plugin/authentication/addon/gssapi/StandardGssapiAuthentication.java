...
public class StandardGssapiAuthentication implements GssapiAuth {
  ...
  public void authenticate(
    ...
    if (gssCredential != null) {
      try {
        gssClientInit(out, in, servicePrincipalName, gssCredential, requestCredentialDelegation);
      ...
      if (!mySubject.getPrincipals().isEmpty()) {
        try {
          PrivilegedExceptionAction<Void> action =
              () -> {
                try {
                  gssClientInit(out, in, servicePrincipalName, null, requestCredentialDelegation);
                } catch (GSSException le) {
                  throw new SQLException("GSS-API authentication exception", "28000", 1045, le);
                }
                return null;
  ...
  private void gssClientInit(
    ...
    GSSContext context =
        manager.createContext(peerName, krb5Mechanism, cred, GSSContext.DEFAULT_LIFETIME);
