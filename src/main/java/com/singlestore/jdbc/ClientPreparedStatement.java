...
public class ClientPreparedStatement extends BasePreparedStatement {
  ...
  public ClientPreparedStatement(
    ...
    super(
        ...
        defaultFetchSize);
    parser = ClientParser.parameterParts(sql);
  ...
  /**
   * Executes a prepared batch statement.
   *
   * @return true if the batch was an insert rewrite statement, false otherwise
   */
  ...
  private ClientMessage[] getClientMessageForRewriteBatchedStatement() {
    RewriteClientParser rewriteClientParser = RewriteClientParser.rewritableParts(parser.getSql());
  ...
  private void executeBatchPipeline() throws SQLException {
    ...
    for (int i = 0; i < batchParameters.size(); i++) {
      packets[i] = new QueryWithParametersPacket(null, parser, batchParameters.get(i), null);
    }
    try {
      results =
          con.getClient()
  ...
  public int[] executeBatch() throws SQLException {
    ...
    try (ClosableLock ignore = lock.closeableLock();
        QueryTimeoutHandler ignore2 = this.con.handleTimeout(queryTimeout)) {
      boolean rewrite = executeInternalPreparedBatch();
      int[] updates = getUpdateCounts(rewrite);
      currResult = results.remove(0);
      return updates;
    } catch (SQLException e) {
      results = null;
  ...
  private int[] getUpdateCounts(boolean rewrite) {
    int[] result;
    if (rewrite) {
      result = new int[batchParameters.size()];
      int resultValue = SUCCESS_NO_INFO;
      if (results.get(0) instanceof OkPacket) {
        int affectedRows = (int) ((OkPacket) results.get(0)).getAffectedRows();
        if (batchParameters.size() == 1 || affectedRows == 0) {
      ...
      for (int i = 0; i < results.size(); i++) {
        if (results.get(i) instanceof OkPacket) {
          result[i] = (int) ((OkPacket) results.get(i)).getAffectedRows();
        } else {
  ...
  public long[] executeLargeBatch() throws SQLException {
    ...
    try (ClosableLock ignore = lock.closeableLock();
        QueryTimeoutHandler ignore2 = this.con.handleTimeout(queryTimeout)) {
      boolean rewrite = executeInternalPreparedBatch();
      long[] updates = getLargeUpdateCounts(rewrite);
      currResult = results.remove(0);
      return updates;
  ...
  private long[] getLargeUpdateCounts(boolean rewrite) {
    long[] result;
    if (rewrite) {
      result = new long[batchParameters.size()];
      long resultValue = SUCCESS_NO_INFO;
      if (results.get(0) instanceof OkPacket) {
        long affectedRows = ((OkPacket) results.get(0)).getAffectedRows();
        if (batchParameters.size() == 1 || affectedRows == 0) {
          resultValue = affectedRows;
        }
      }
      ...
      for (int i = 0; i < results.size(); i++) {
        if (results.get(i) instanceof OkPacket) {
          result[i] = ((OkPacket) results.get(i)).getAffectedRows();
        } else {
          result[i] = SUCCESS_NO_INFO;
        }
      }
